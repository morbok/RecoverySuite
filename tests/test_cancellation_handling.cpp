...
// Test that recovery operations properly handle cancellation before starting
void test_cancellation_before_operation() {
    std::cout << "Testing cancellation handling before operation starts..." << std::endl;
    ...
    // Test parameters with cancellationStateIsValid = false (simulating cancellation requested)
    ...
    bool cancellationStateIsValid = false; // CANCELLATION REQUESTED
    ...
    // Test FilesystemAnalyzer
    {
        ...
        if (success) {
            std::cerr << "ERROR: FilesystemAnalyzer should have been cancelled!" << std::endl;
            mockReader->close();
            return;
        } else {
            std::cout << "[PASS] FilesystemAnalyzer correctly handled pre-operation cancellation" << std::endl;
        }
    }
    ...
    // Test MetadataRecovery
    {
        ...
        if (success) {
            std::cerr << "ERROR: MetadataRecovery should have been cancelled!" << std::endl;
            mockReader->close();
            return;
        } else {
            std::cout << "[PASS] MetadataRecovery correctly handled pre-operation cancellation" << std::endl;
        }
    }
    ...
    // Test FileRecovery
    {
        ...
        if (success) {
            std::cerr << "ERROR: FileRecovery should have been cancelled!" << std::endl;
            mockReader->close();
            return;
        } else {
            std::cout << "[PASS] FileRecovery correctly handled pre-operation cancellation" << std::endl;
        }
    }
    ...
    // Test CarvingEngine
    {
        ...
        if (success) {
            std::cerr << "ERROR: CarvingEngine should have been cancelled!" << std::endl;
            mockReader->close();
            return;
        } else {
            std::cout << "[PASS] CarvingEngine correctly handled pre-operation cancellation" << std::endl;
        }
    }
...
// Test that recovery operations properly handle valid cancellation state (no cancellation)
void test_valid_cancellation_state() {
    std::cout << "Testing recovery operations with valid cancellation state..." << std::endl;
    ...
    // Test parameters with cancellationStateIsValid = true (no cancellation requested)
    bool sourceIsReadOnly = true;
    bool destinationDiffersFromSource = true;
    uint64_t startSector = 0;
    uint64_t numSectors = 1;
    ...
    bool outputPathIsValid = true;
    bool arithmeticCannotOverflow = true;
    bool cancellationStateIsValid = true; // NO CANCELLATION

    // Test FilesystemAnalyzer - should proceed with validation (may fail for other reasons, but not cancellation)
    {
        recoverysuite::recovery::FilesystemAnalyzer analyzer(
            capabilityRegistry,
            safetyPolicy,
        ...
        // but it should not fail due to cancellation
        // We're mainly checking that it doesn't fail specifically because of cancellation
        std::cout << "[PASS] FilesystemAnalyzer processed operation with valid cancellation state" << std::endl;
    }
...
int main() {
    std::cout << "Testing RecoverySuite Cancellation Handling..." << std::endl;

    try {
        test_cancellation_before_operation();
        test_valid_cancellation_state();

        std::cout << "All cancellation handling tests passed!" << std::endl;
        return 0;
